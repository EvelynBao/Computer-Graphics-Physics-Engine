...
struct point mouseForce = { 0.0, 0.0, 0.0 };  // user force
int mouseX, mouseY;
...
void mouseMotionDrag(int x, int y)
{
    ...
    if (g_iLeftMouseButton) // drag force
    {
        double forceFactor = 0.005;
        // store the force to mouseForce
        int dx = x - mouseX;
        int dy = y - mouseY;
        mouseForce.x = dx * forceFactor;
        mouseForce.y = -dy * forceFactor;
        mouseForce.z = 0.0;

        //printf("[mouseMotionDrag] Dragging: dx=%d, dy=%d → Force: (%.2lf, %.2lf, %.2lf)\n",
        //    dx, dy, mouseForce.x, mouseForce.y, mouseForce.z);
  ...
  switch (button)
  {
    case GLUT_LEFT_BUTTON:
      ...
      if (state == GLUT_DOWN) {
          ...
          //printf("[mouseButton] Clicked at: (%d, %d)\n", mouseX, mouseY);
...
// get matrix and 2d coordinate of mouse click
void getMouseClickInfo(int* x, int* y, GLdouble* modelview, GLdouble* projection, GLint* view) {
    *x = mouseX;
    *y = mouseY;
    for (int i = 0; i < 16; i++) {
        modelview[i] = modelviewMatrix[i];
        projection[i] = projectionMatrix[i];
    }
    for (int i = 0; i < 4; i++) {
        view[i] = viewport[i];
    }
...
void keyboardFunc (unsigned char key, int x, int y)
{
  switch (key)
  {
    ...
    case 'm': // to perform mounse drag force
        Theta = 0;
        Phi = 0;
        viewingMode = 0;
        break;
