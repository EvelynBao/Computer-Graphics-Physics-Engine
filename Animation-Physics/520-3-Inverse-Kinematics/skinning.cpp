...
void Skinning::applySkinning(const RigidTransform4d * jointSkinTransforms, double * newMeshVertexPositions) const
{
  ...
  for(int i=0; i<numMeshVertices; i++)
  {
      ...
      for (int j = 0; j < numJointsInfluencingEachVertex; j++) {
          ...
          Vec3d transformed = jointSkinTransforms[jointID].transformPoint(restPosition);
          newPosition += weight * transformed;
      }

      newMeshVertexPositions[3 * i + 0] = newPosition[0];
      newMeshVertexPositions[3 * i + 1] = newPosition[1];
      newMeshVertexPositions[3 * i + 2] = newPosition[2];
